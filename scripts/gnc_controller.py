...
class CONTROLLER:
    def __init__(self):
        ...
        if self.namespace == "/":
            ...
            rospy.loginfo("Using {} namespace".format(self.namespace))

        self.local_position_publisher = rospy.Publisher(
            name="{}mavros/setpoint_position/local".format(self.namespace),
            data_class=PoseStamped,
            queue_size=10,
        )

        self.current_position_subsciber = rospy.Subscriber(
            name="{}mavros/global_position/local".format(self.namespace),
            data_class=Odometry,
            queue_size=10,
        ...
        self.state_subscriber = rospy.Subscriber(
            name="{}mavros/state".format(self.namespace),
            data_class=State,
            queue_size=10,
            callback=self.state_callback,
        )

        rospy.wait_for_service("{}mavros/cmd/arming".format(self.namespace))

        self.arming_client = rospy.ServiceProxy(
            name="{}mavros/cmd/arming".format(self.namespace), service_class=CommandBool
        )

        rospy.wait_for_service("{}mavros/cmd/land".format(self.namespace))

        self.land_client = rospy.ServiceProxy(
            name="{}mavros/cmd/land".format(self.namespace), service_class=CommandTOL
        )

        rospy.wait_for_service("{}mavros/cmd/takeoff".format(self.namespace))

        self.takeoff_client = rospy.ServiceProxy(
            name="{}mavros/cmd/takeoff".format(self.namespace), service_class=CommandTOL
        )

        rospy.wait_for_service("{}mavros/set_mode".format(self.namespace))

        self.set_mode_client = rospy.ServiceProxy(
            name="{}mavros/set_mode".format(self.namespace), service_class=SetMode
        )

        rospy.wait_for_service("{}mavros/cmd/command".format(self.namespace))

        self.command_client = rospy.ServiceProxy(
            name="{}mavros/cmd/command".format(self.namespace), service_class=CommandLong
        )

        rospy.loginfo("Controller Initialization Complete.")
        time.sleep(0.5)
    ...
    def pose_callback(self, message):
        ...
        # check the order
        self.pitch, self.roll, self.yaw = self.get_euler_angles()
    ...
    def get_quaternion_from_yaw(self, yaw):
        qx = np.sin(self.roll/2) * np.cos(self.pitch/2) * np.cos(yaw/2) - \
            np.cos(self.roll/2) * np.sin(self.pitch/2) * np.sin(yaw/2)
        qy = np.cos(self.roll/2) * np.sin(self.pitch/2) * np.cos(yaw/2) + \
            np.sin(self.roll/2) * np.cos(self.pitch/2) * np.sin(yaw/2)
        qz = np.cos(self.roll/2) * np.cos(self.pitch/2) * np.sin(yaw/2) - \
            np.sin(self.roll/2) * np.sin(self.pitch/2) * np.cos(yaw/2)
        qw = np.cos(self.roll/2) * np.cos(self.pitch/2) * np.cos(yaw/2) + \
            np.sin(self.roll/2) * np.sin(self.pitch/2) * np.sin(yaw/2)
        return [qx, qy, qz, qw]
    ...
    def set_mode(self, mode):
        set_mode_srv = SetModeRequest(0, mode)
        response = self.set_mode_client(set_mode_srv)
        if response.mode_sent:
            rospy.loginfo("Mode was set to {} successfully".format(mode))
            return 0
        else:
            rospy.logerr("Failed to set mode to {}".format(mode))
            return -1

    def set_waypoint(self, x, y, psi, z = 2):

        yaw = np.deg2rad(psi) + np.pi/2
        local_desired_heading = yaw
        [qx, qy, qz, qw] = self.get_quaternion_from_yaw(yaw)

        self.waypoint.pose.orientation = Quaternion(qx, qy, qz, qw)
        self.waypoint.pose.position = Point(x, y, z)
        rospy.loginfo("Destination set to {}, {}, {}".format(self.waypoint.pose.position.x, self.waypoint.pose.position.y, self.waypoint.pose.position.z))
        self.local_position_publisher.publish(self.waypoint)

    def arm(self):
        self.set_waypoint(0, 0, 0, 0)
        for _ in range(100):
            self.local_position_publisher.publish(self.waypoint)
            rospy.sleep(0.01)
    ...
    def takeoff(self, takeoff_altitude):
        self.arm()  # ensures drone is armed before takeoff
        takeoff_srv = CommandTOLRequest(0, 0, 0, 0, takeoff_altitude)
    ...
    def check_waypoint_reached(self, position_tolerance=0.3, head_tolerance=2.5):
        self.local_position_publisher.publish(self.waypoint)

        dx = abs(
            self.waypoint.pose.position.x - self.current_pose.pose.pose.position.x
        )
        dy = abs(
            self.waypoint.pose.position.y - self.current_pose.pose.pose.position.y
        )
        dz = abs(
            self.waypoint.pose.position.z - self.current_pose.pose.pose.position.z
        )

        dMag = sqrt(pow(dx, 2) + pow(dy, 2) + pow(dz, 2))

        cosErr = np.cos(self.yaw) - np.cos(self.local_desired_heading)

        sinErr = np.sin(self.yaw) - np.sin(self.local_desired_heading)

        dHead = sqrt(pow(cosErr, 2) + pow(sinErr, 2))

        # dHead = abs(np.rad2deg(self.yaw) - np.rad2deg(self.local_desired_heading))

        rospy.loginfo(dHead)
        if dMag < position_tolerance and dHead < head_tolerance :
            return 1
        else:
            return 0
...
#     def set_destination(self, x, y, psi, z = 2):
